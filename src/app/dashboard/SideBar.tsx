...
export default function SideBar() {
	...
	const [unassignedOrdersCount, setUnassignedOrdersCount] =
		useState<number>(0)

	// Fetch unassigned orders count from Orders collection
	useEffect(() => {
		const ordersQuery = query(
			collection(db, "Orders"),
			where("status", "in", ["accepted", "reviewed"])
		)

		const unsubscribe = onSnapshot(
			ordersQuery,
			(snapshot) => {
				setUnassignedOrdersCount(snapshot.docs.length)
			},
			(error) => {
						...
						<div className="overflow-y-auto overflow-x-hidden flex-grow">
							<ul className="flex flex-col py-4 space-y-1">
								...
								<Link href={"/dashboard/order"}>
									<li>
										<div className="relative flex flex-row items-center h-11 focus:outline-none hover:bg-gray-50 text-gray-600 hover:text-gray-800 border-l-4 border-transparent hover:border-indigo-500 pr-6">
											<span className="inline-flex justify-center items-center ml-4 relative">
												<svg
													className="w-5 h-5"
													fill="none"
													stroke="currentColor"
