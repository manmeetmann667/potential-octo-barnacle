...
export default function ProductsPage() {
  ...
  const handleAddProduct = async () => {
    if (!storeId || !formData.catalogueProductName.trim() || !formData.catalogueCategoryId || !formData.productDescription || !formData.productImageUrl) {
      toast.error("Store ID, product name, description, image URL, and category are required!");
      return;
    }
    try {
      const newProduct = {
    ...
    if (storeId) {
      try {
        ...
        const filteredProducts = categoryId
          ? fetchedProducts.filter((product: Product) => product.catalogueCategoryId === categoryId)
          : fetchedProducts;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 w-96">
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, catalogueCategoryId: e.target.value })}
              className="border-2 w-full p-2 rounded-lg mb-2"
            >
              <option value="">Select Category</option>
              {categories.map((category) => (
